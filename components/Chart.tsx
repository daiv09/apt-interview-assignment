...
export default function Chart({ chartHistory, isExpanded, onToggle }: ChartProps) {
    ...
    return (
        <section className="w-full max-w-4xl bg-white border border-gray-200 rounded-xl p-5 mb-6 shadow-sm">
            ...
            <div
                className={`w-full transition-all duration-300 ease-in-out overflow-hidden min-w-0 ${isExpanded ? "min-h-[256px] h-64 mt-4 opacity-100" : "h-0 opacity-0 pointer-events-none"
                    }`}
            ...
            >
                {hasMounted && chartHistory.length > 0 ? (
                    ...
                    <div className="w-full h-full min-h-[256px] flex items-center justify-center border border-dashed border-gray-200 rounded-lg bg-gray-50/50">
